...
session_start();
if (isset($_POST['emp_id']) && isset($_POST['time_out']) && isset($_POST['logs_id'])) {
    ...
    while ($result4 = $user_data->fetch(PDO::FETCH_ASSOC)) {
        ...
        $emp_id = $_POST['emp_id'];
        $sched_id = "A1";
        $punch_out = date('H:i:s'); // 24-hour format: HH:MM:SS

        $time_logs_sql = "INSERT INTO tbl_employee_timelogs SET 
        emp_id = :emp_id,
        date_logs = :date_logs,
        date_punch_out = :date_punch_out,
        logs_id = :logs_id,
        punch_out = :punch_out,
