...
$get_employee_sql = "SELECT * FROM tbl_weekly_payslip where emp_id='$emp_id' AND status='ACTIVE' ORDER BY id DESC ";
...
$getAllIndividual = "SELECT emp_id,firstname,middlename,lastname,suffix FROM tbl_employee_info where status='ACTIVE' AND";
...
if (!empty($requestData['search']['value'])) {
    ...
    $countfilter = "SELECT emp_id,firstname,middlename,lastname,suffix FROM tbl_employee_info where status='ACTIVE' AND";
...
while ($row = $getIndividualData->fetch(PDO::FETCH_ASSOC)) {
    ...
    $nestedData[] = $row["payroll_id"];
     $nestedData[] = $row["date_from"].' - '.$row["date_to"];
    $nestedData[] = $row["total_emp_deduction"];
    $nestedData[] = $row["emp_total_netpay"];
